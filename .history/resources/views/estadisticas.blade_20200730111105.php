                 ...
                 <div class="row">
                    ...
                    @foreach ($visitasestado as $inmueble)
                        @if ($inmueble->estado == 1)
                            <li color="red">{{ $inmueble->calle }} - {{ $inmueble->nombre }}: Pendiente</li>
                        @endif
                         @if ($inmueble->estado == 2)
                            <li color="yellow">{{ $inmueble->calle }} - {{ $inmueble->nombre }}: Seguimiento</li>
                        @endif
                         @if ($inmueble->estado == 3)
                            <li color="blue">{{ $inmueble->calle }} - {{ $inmueble->nombre }}: Visitando</li>
                        @endif
                         @if ($inmueble->estado == 4)
                            <li color="green">{{ $inmueble->calle }} - {{ $inmueble->nombre }}: Ofertado</li>
                        @endif
                    @endforeach
                    </ul>
